...
namespace ComricFraudCalculatorBackend.Authentication;

/// <summary>
/// Local development auth bypass. Enabled only when LocalDevelopment:UseDevAuth is true.
    ...
    UrlEncoder encoder) : AuthenticationHandler<AuthenticationSchemeOptions>(options, logger, encoder)
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        var tenantId = Request.Headers.TryGetValue("X-Dev-TenantId", out var tenantHeader)
            ? tenantHeader.ToString()
            : DefaultTenantId.ToString();

        var scopes = Request.Headers.TryGetValue("X-Dev-Scopes", out var scopeHeader)
